...
Ext.ss.form.ComboBox = Ext.extend(Ext.form.ComboBox, {
	...
	onRender: function (ct, position) {
		...
		if (this.showDescription) {
			...
			if(this.descriptionReadOnly) {
				Ext.apply(this.defaultDescriptionConfig,{tabIndex: -1});
			}
	...
	setValue: function (v) {
		...
		if (this.valueField) {
			...
			if (r) {
				...
				if (!Ext.isEmpty(v)) {
					this.value = v;
					this.doLoadValue({ value: v },function(records, options, success){
	...
	assertValue : function(options){
        ...
        if(!rec  && !Ext.isEmpty(this.value)) {
        	this.doLoadValue({value:this.value},function(records, options, success){
		        ...
		        if(!rec) {
		        	this.doLoadValue({display:val},function(records, options, success){
		        		rec = Ext.isArray(records)?records[0]:records;
