...
Ext.ss.grid.CheckboxSelectionModel = Ext.extend(Ext.grid.AbstractSelectionModel, {
    ...
    selectRecord : function(record, keepExisting){
        ...
        var ds = this.grid.store;
        this.selectRow(ds.indexOf(record));
    },
    highlightRecords : function(records, keepExisting){
        if(!keepExisting){
            this.clearSelections();
        }
        var ds = this.grid.store,
    ...
    highlightRecord : function(record, keepExisting, scrollToView){
        if(!keepExisting){
            this.clearSelections();
        }
        var ds = this.grid.store,row = ds.indexOf(record);
        this.highlightRow(row);
        if(scrollToView) {
        	this.grid.view.ensureVisible(row, 0, true);
        }
    },
    acceptsNav : function(row, col, cm){
    	var accepts = !cm.isHidden(col) && cm.isCellEditable(col, row),
    		grid = this.grid;
    	if(accepts) {
			...
			accepts = (grid.fireEvent("beforeedit", e) !== false && !e.cancel); 
    	}
        return accepts;
    },
    onEditorKey : function(field, e){
        ...
        function processOnEditorKey(newCell, r, c, last, g) {
        	if(newCell){
	            r = newCell[0];
	            c = newCell[1];
        ...
        if(k == e.TAB){
	        ...
	        if (ed.editing && ed.field.assertValue) {
	            ed.field.assertValue({
	            	callback : function() {
	            		ed.completeEdit();
			            ...
			            processOnEditorKey.call(this, newCell, r, c, last, g);
	            	},
	            	scope: this
	            });
	            return;
    ...
    onEditorSelect: function(row, lastRow){
        if(lastRow != row){
            this.highlightRow(row); // *** highlight newly-selected cell and update selection
        }
    },
    destroy : function(){
        if(this.rowNav){
